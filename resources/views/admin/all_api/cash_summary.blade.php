...
@section('content')
  ...
  <div class="container-fluid">
                    ...
                    <div class="row">
                                        <div class="col-md-10">
                                   <h1 class="h3 mb-2 text-gray-800">Total Cash Summary (<?php echo date('M Y');?>)</h1>
                                    </div>
                                     <div class="col-md-2">
                                ...
                                <table class="export_example1">
                                    ...
                                    <tbody>
                                        ...
                                        @if(!empty($total_payment))
                                            ...
                                            @php $i=0; @endphp
                                         @foreach($total_payment as $kexy => $info)
...
$sytem_amt = $info->total_amount - $adjustment_pay;

                                                    @endphp
                                           <a href="{{ route('cash_site_summary',$info->site_id) }}" target="_blank"> {{ $info->total_amount - $adjustment_pay ?? '0'}} </a>
                                           ...
                                           $sum_received = \App\Models\PaymentCollect::where('payment_adjustment','0')->where('site_id',$info->site_id)->where('month',date('m'))->where('year',date('Y'))->get()->sum('cash_received');   
                                            @endphp  
                                             <a href="{{ route('cash_received_summary',$info->site_id) }}" target="_blank"> {{ $sum_received }} </a></td>
                                                    <td> @php
                                           $sum_due = $sytem_amt - $sum_received;
                                     ...
                                     <tfoot>
                                        <tr>
                                            <th>--</th>
                                             <th>--</th>
                                           <th>{{ $total_system_amount - $payment_adjustment ?? '0'}}</th>
                                           <!--<th>{{ $total_system_amount ?? '0'}}</th>-->
                                           <th>{{ $payment_received ?? '0' }}</th>
                                           <th>--</th>
                                        </tr>
                                    </tfoot>
                                    </tbody>
