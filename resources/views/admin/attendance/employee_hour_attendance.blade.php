...
@if(Auth::user()->role == '1')
  ...
  <!-- Page Heading -->
                    <div class="row">
                                        <div class="col-md-10">
                                   <h1 class="h3 mb-2 text-gray-800">Hour Wise Attendance List</h1>
                                    </div>
                                     <div class="col-md-2">
                                         @php $id = Request::segment(3); @endphp
                                     <a href="{{ route('employee_attendance',$id) }}" class="btn btn-dark btn-icon-split">
...
$userlist = App\Models\Attendance::select('*')->where('employee_id', $ids)->where('day', $mtoday)->where('year', $y_day)->where('month', $m_day)->first();
            $time1 = new DateTime($userlist->clock_in);
            $time2 = new DateTime($userlist->clock_out);
            $time_diff = $time1->diff($time2);
            $hourss = $time_diff->h.':0';
 $emp_total_hour = App\Models\Attendance::where('month',$m_day)->where('year',$y_day)->where('employee_id', $ids)->sum('total_hours');
           
@endphp   
                                                     <td><a href="#" data-toggle="modal" data-target="#myModal_{{$mtoday}}"><b>{{ $hourss }}</b></a></td>
                                                     <!-- Modal -->
    ...
    $hourss = $time_diff->h.' hours';
                    ...
                    <div class="border rounded p-3 bg-light">
                        ...
                        @endif
                                            ...
                                            <td><div class="btn btn-primary btn-sm rounded-pill"> {{$emp_total_hour}} hrs</div></td>
                                            @endif
  ...
  <div class="container-fluid">
                    ...
                    <div class="row">
                                   ...
                                   <h1 class="h3 mb-2 text-gray-800">All Attendance List</h1>
                                    ...
                                    <div class="col-md-3">
                                          <a href="{{ route('all_employee_hour_attendance') }}" class="btn btn-primary btn-icon-split">
